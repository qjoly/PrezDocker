...
À l'aide des différents outils autour de Docker, nous pouvons installer des applications <i> comme WordPress, Nextcloud, </i> et ce de manière quasiment automatique. <br>
...
<div style="font-size: 0.7em;">
À l'aide des différents outils autour de Docker, nous pouvons installer une application <i> comme WordPress, Nextcloud, </i> et ce de manière quasiment automatique. <br>
L'idée est alors de se passer des démarches d'installation en les appliquant automatiquement dans un conteneur.
</div>
...
# Résumé de la présentation de l'outil 
...
- Un conteneur est une machine virtuelle plus légère, plus rapide mais moins polyvalente <div style="font-size: 0.4em;"> du fait de à la paravirtualisation </div>.
...
On peut d'ailleurs aisément créer notre propre image. <br> *Nous verrons cela plus tard*.

Cependant l'aspect communautaire de Docker nous permet aussi de récupérer les images d'autres personnes pour en faire des conteneurs !
...
Un Dockerfile est la "recette" pour créer une image. On choisit notre distribution comme 'couche de base', on y ajoute nos dépendances ainsi que les programmes nécessaires au bon fonctionnement de l'application que l'on souhaite dockeriser.
...
Nous aurions pu mettre le même port (8080:8080), vous êtes libres de choisir le port *de votre machine* (notre app utilise uniquement le port 8080 dans le conteneur).
